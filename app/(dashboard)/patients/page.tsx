...
export default function PatientsPage() {
  ...
  async function loadChat(patientId: string) {
    ...
    try {
      ...
      const unread = res.items.filter((m) => !m.is_read && m.sender_role === "patient").length;
    ...
    <div className="space-y-4">
      ...
      <Dialog open={Boolean(selectedId)} onOpenChange={(open) => (!open ? setSelectedId(null) : undefined)}>
        <DialogContent className="max-w-4xl p-0">
          ...
          <div className="p-6">
            {detailLoading ? <div className="text-sm text-muted-foreground">Loading</div> : null}
            ...
            {!detail ? null : tab === "dossier" ? (
              <div className="grid gap-4 md:grid-cols-2">
                <Card>
                  ...
                  <CardContent className="grid gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <div>
                        {(detail.patient.emergency_contact_name || detail.patient.emergency_contact_phone)
                          ? `${detail.patient.emergency_contact_name ?? ""} ${detail.patient.emergency_contact_phone ?? ""}`.trim()
                          : "—"}
                      </div>
                    </div>
                  </CardContent>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base">Antécédents</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3 text-sm">
                    <div>
                      <div className="text-muted-foreground">Allergies</div>
                      <div>{detail.patient.allergies || "—"}</div>
                    </div>
                    <div>
                      <div className="text-muted-foreground">Historique</div>
                      <div>{detail.patient.medical_history || "—"}</div>
                    </div>
                    <div>
                    ...
                    <div className="space-y-3">
                      {detail.consultations?.map((c: any) => (
                        <div key={c.id} className="rounded-xl border border-border p-3">
                          ...
                          <div className="mt-1 font-medium text-sm">{c.reason ?? "—"}</div>
                          <div className="text-sm text-muted-foreground">{c.diagnosis ?? ""}</div>
                        </div>
                      ))}
                      {!detail.consultations?.length ? <div className="text-sm text-muted-foreground">Empty</div> : null}
                    </div>
                  </CardContent>
                </Card>
                        ...
                        onClick={async () => {
                          ...
                          try {
                            await apiFetch(`/api/patients/${selectedId}/consultations`, {
                              method: "POST",
                              body: JSON.stringify({
                                reason: newConsultation.reason || undefined,
                                exam: newConsultation.exam || undefined,
                                diagnosis: newConsultation.diagnosis || undefined,
                                treatment: newConsultation.treatment || undefined,
              ...
              (() => {
                ...
                return (
                  <div className="space-y-4">
                        ...
                        <CardContent className="h-64">
                          <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={chartData}>
                              ...
                              <YAxis />
                              <Tooltip />
                              <Line type="monotone" dataKey="weight" stroke="#9333ea" dot={false} />
                            </LineChart>
                          </ResponsiveContainer>
                        </CardContent>
                      </Card>
                    ...
                    {rows.length === 0 ? <div className="text-sm text-muted-foreground">Empty</div> : null}
                  </div>
                );
              })()
            ) : tab === "documents" ? (
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                        ...
                        <div key={d.id} className="flex items-center justify-between gap-3 rounded-xl border border-border p-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                if (!selectedId) return;
                                await apiFetch(`/api/patients/${selectedId}/documents/${d.id}`, { method: "DELETE" });
                                await loadDetail(selectedId);
                              }}
                            >
                              Supprimer
                      ...
                      {!detail.documents?.length ? <div className="text-sm text-muted-foreground">Empty</div> : null}
                    </div>
                  </CardContent>
                </Card>
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            await apiUpload(`/api/patients/${selectedId}/documents`, form);
                            setDocFile(null);
                            await loadDetail(selectedId);
                          } finally {
              ...
              <Card>
                ...
                <CardContent>
                  {chatLoading ? <div className="text-sm text-muted-foreground">Loading</div> : null}
                  <div className="mb-3 h-[45vh] overflow-auto rounded-xl border border-border bg-card p-3">
                    <div className="space-y-2">
                      {chatItems.map((m) => {
                        ...
                        return (
                          <div key={m.id} className={cn("flex", mine ? "justify-end" : "justify-start")}>
                            ...
                            >
                              <div className={cn("text-[10px] opacity-70", mine ? "text-primary-foreground" : "text-muted-foreground")}>
                                {m.sender_role} • {new Date(m.created_at).toLocaleTimeString()}
                              </div>
                      ...
                      {chatItems.length === 0 ? <div className="text-sm text-muted-foreground">Empty</div> : null}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      onClick={async () => {
                        ...
                        if (!content) return;
                        const channel = `patient:${selectedId}`;
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base">Compte patient</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    {accountLoading ? <div className="text-sm text-muted-foreground">Loading</div> : null}
                    ...
                    {account ? (
                      <div className="space-y-2 text-sm">
                        ...
                        <div>
                          ...
                          <div className="font-medium">{account.is_active ? "Actif" : "Désactivé"}</div>
                        </div>
                        <div className="flex gap-2">
                          <Button
                            variant="outline"
                      ...
                      <div className="text-sm text-muted-foreground">Aucun accès patient</div>
                    )}
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base">Créer / Reset</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <Input
                    ...
                    <div className="flex justify-end">
                      <Button
                        disabled={!accountForm.username.trim() || accountForm.password.length < 6}
                        onClick={async () => {
                          if (!selectedId) return;
                          setAccountError(null);
                          try {
                            ...
                            setAccountForm((s) => ({ ...s, password: "" }));
                            await loadAccount(selectedId);
                          } catch {
                            setAccountError("Impossible de créer/modifier le compte");
                          }
                        }}
                      >
                        Enregistrer
                    ...
                    {account ? (
                      <>
                        ...
                        <Input
                          placeholder="Reset mot de passe"
                          type="password"
                          value={resetPassword}
                        ...
                        <div className="flex justify-end">
                          <Button
                            variant="outline"
                            disabled={resetPassword.length > 0 && resetPassword.length < 6}
                            onClick={async () => {
                              if (!selectedId || !resetPassword) return;
                              await apiFetch(`/api/settings/patient-accounts/${selectedId}`, {
                                method: "PUT",
