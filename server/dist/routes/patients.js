...
exports.patientsRouter.delete('/:id', auth_js_1.authenticateToken, (0, permissions_js_1.requirePermission)('patients', 'delete'), async (req, res) => {
    try {
        const result = await (0, pool_js_1.query)('DELETE FROM patients WHERE id = $1 RETURNING id', [req.params.id]);
...
exports.patientsRouter.put('/:id/consultations/:consultationId', auth_js_1.authenticateToken, (0, permissions_js_1.requirePermission)('consultations', 'write'), async (req, res) => {
    const { motif, ecole, examen, diagnostic, traitement, note, date } = req.body;
    try {
        await (0, pool_js_1.query)('UPDATE consultations SET date=$1, motif=$2, ecole=$3, examen=$4, diagnostic=$5, traitement=$6, note=$7 WHERE id=$8 AND patient_id=$9', [date, motif, ecole, examen, diagnostic, traitement, note, req.params.consultationId, req.params.id]);
