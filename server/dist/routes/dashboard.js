...
dashboardRouter.get('/summary', authenticateToken, async (req, res) => {
    try {
        ...
        const newPatients = await query("SELECT count(*)::int as count FROM patients WHERE created_at >= date_trunc('month', CURRENT_DATE)");
        const appointmentsToday = await query(`SELECT a.*, p.first_name, p.last_name, p.severity_status, p.pathology
             FROM appointments a 
             JOIN patients p ON a.patient_id = p.id 
             WHERE a.date = CURRENT_DATE 
             ORDER BY a.time ASC`);
        const appointmentsCountToday = appointmentsToday.rows.length;
        const appointmentsUrgent = await query('SELECT count(*)::int as count FROM appointments WHERE date = CURRENT_DATE AND status = \'urgent\'');
        const appointmentsPlanned = await query("SELECT count(*)::int as count FROM appointments WHERE date >= CURRENT_DATE");
        const appointmentsCompleted = await query("SELECT count(*)::int as count FROM appointments WHERE status = 'completed' AND date >= date_trunc('month', CURRENT_DATE)");
        const unreadMessages = await query('SELECT count(*)::int as count FROM chat_messages WHERE channel = \'staff\' AND is_read = FALSE');
        const criticalAlerts = await query(`SELECT v.*, p.first_name, p.last_name, p.severity_status 
             FROM vital_entries v 
             JOIN patients p ON v.patient_id = p.id 
             WHERE v.sp02 < 94 OR v.heart_rate > 100 
             ORDER BY v.recorded_at DESC LIMIT 10`);
        const recentActivity = await query(`(SELECT id, created_at as event_at, 'consultation' as event_type, patient_id, motif as label 
        ...
        const activityWithNames = await Promise.all(recentActivity.rows.map(async (act) => {
            const p = await query('SELECT first_name, last_name FROM patients WHERE id = $1', [act.patient_id]);
            return { ...act, ...p.rows[0] };
        }));
        res.json({
