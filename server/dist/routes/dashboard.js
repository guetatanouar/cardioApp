...
exports.dashboardRouter.get('/summary', auth_js_1.authenticateToken, async (req, res) => {
    try {
        ...
        const newPatients = await (0, pool_js_1.query)("SELECT count(*)::int as count FROM patients WHERE created_at >= date_trunc('month', CURRENT_DATE)");
        const appointmentsToday = await (0, pool_js_1.query)(`SELECT a.*, p.first_name, p.last_name, p.severity_status, p.pathology
             FROM appointments a 
        ...
        const appointmentsPlanned = await (0, pool_js_1.query)("SELECT count(*)::int as count FROM appointments WHERE date >= CURRENT_DATE");
        const appointmentsCompleted = await (0, pool_js_1.query)("SELECT count(*)::int as count FROM appointments WHERE status = 'completed' AND date >= date_trunc('month', CURRENT_DATE)");
