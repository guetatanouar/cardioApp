...
function generateReport(patient: PatientInfo, vitals: VitalEntry[], documents: DocInfo[]) {
...
analyseRouter.get('/patients', authenticateToken, requirePermission('documents'), async (_req, res) => {
  try {
    const patients = await query(
      `SELECT p.id, p.first_name, p.last_name, p.severity_status, p.pathology,
              COALESCE(
                json_agg(
                  json_build_object(
  ...
  try {
    ...
    if (uploadedFile) {
      ...
      await query(
        ...
        [docId, patientId, uploadedFile.originalname, 'analyse', String(uploadedFile.size), uploadedFile.path]
      );
      docIds.push(docId);
    }

    const patientResult = await query(
      'SELECT id, first_name, last_name, pathology, severity_status, blood_type, date_of_birth FROM patients WHERE id = $1',
      [patientId]
    );
    const patient = patientResult.rows[0] as PatientInfo | undefined;

    const vitalsResult = await query(
      'SELECT systolic, diastolic, heart_rate, sp02, weight FROM vital_entries WHERE patient_id = $1 ORDER BY recorded_at DESC LIMIT 1',
      [patientId]
    );
    const vitals = vitalsResult.rows as VitalEntry[];

    const docsResult = await query(
      'SELECT id, name, category FROM documents WHERE id = ANY($1::text[])',
      [docIds]
    );
    const docs = docsResult.rows as DocInfo[];
    ...
    await query(
      `INSERT INTO analysis_reports (id, patient_id, document_ids, report_content, created_by)
       VALUES ($1, $2, $3, $4, $5)`,
      [reportId, patientId, JSON.stringify(docIds), JSON.stringify(report), (req as any).user?.id || null]
    );
...
analyseRouter.post('/analyze-existing', authenticateToken, requirePermission('documents'), async (req, res) => {
  try {
    ...
    await query(
      `INSERT INTO analysis_reports (id, patient_id, document_ids, report_content, created_by)
       VALUES ($1, $2, $3, $4, $5)`,
      [reportId, patientId, JSON.stringify(documentIds), JSON.stringify(report), (req as any).user?.id || null]
    );
