...
documentTemplatesRouter.get('/:id', authenticateToken, async (req, res) => {
    try {
        const result = await query(
            'SELECT dt.*, u.full_name as author_name FROM document_templates dt LEFT JOIN users u ON dt.created_by = u.id WHERE dt.id = $1',
            [req.params.id]
        );
...
documentTemplatesRouter.post('/', authenticateToken, requirePermission('templates', 'write'), async (req, res) => {
    ...
    try {
        await query(
            'INSERT INTO document_templates (id, name, description, category, content, created_by) VALUES ($1, $2, $3, $4, $5, $6)',
            [id, name, description, category, JSON.stringify(content), user?.id]
        );
        createNotification({
...
documentTemplatesRouter.put('/:id', authenticateToken, requirePermission('templates', 'write'), async (req, res) => {
    ...
    try {
        const result = await query(
            'UPDATE document_templates SET name = $1, description = $2, category = $3, content = $4, updated_at = NOW() WHERE id = $5 RETURNING id',
            [name, description, category, JSON.stringify(content), req.params.id]
        );
        if (result.rows.length === 0) return res.status(404).json({ message: 'Template not found' });
...
documentTemplatesRouter.post('/:id/use', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (tplResult.rows.length === 0) return res.status(404).json({ message: 'Template not found' });
