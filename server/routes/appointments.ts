...
appointmentsRouter.post('/', authenticateToken, requirePermission('appointments', 'write'), async (req, res) => {
    ...
    const overlapCheck = await query(
        `SELECT id FROM appointments
         WHERE date = $1
           ...
           AND (time::time, time::time + (duration || ' minutes')::interval) OVERLAPS ($2::time, $3::time)`,
    ...
    try {
        await query(
            ...
            [id, patientId, dateStr, timeStr, duration, type, status ?? 'scheduled', reason ?? null, notes ?? null]
        );
...
appointmentsRouter.put('/:id', authenticateToken, requirePermission('appointments', 'write'), async (req, res) => {
    ...
    try {
        if (patientId) {
            ...
            const overlapCheck = await query(
                `SELECT id FROM appointments
                 WHERE date = $1
                   AND id != $2
                   AND status != 'cancelled'
                   AND (time::time, time::time + (duration || ' minutes')::interval) OVERLAPS ($3::time, $4::time)`,
                [date, req.params.id, time, endTimeStr]
            );
            ...
            await query(
                'UPDATE appointments SET patient_id=$1, date=$2, time=$3, duration=$4, type=$5, reason=$6, notes=$7 WHERE id=$8',
                [patientId, date, time, duration, type, reason ?? null, notes ?? null, req.params.id]
            );
        } else {
