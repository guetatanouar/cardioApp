...
async function notifyUnreadMessages() {
  try {
    const result = await query(
      `SELECT cm.*, u.full_name as sender_name, p.email as patient_email
    ...
    const staffUsers = (await query(
      `SELECT id, full_name, email, role FROM users WHERE role IN ('admin', 'secretaire')`
    )).rows;
    const notifiedIds: string[] = [];

    for (const msg of messages) {
      if (msg.channel === 'staff') {
        const recipients = staffUsers.filter((u: any) => u.id !== msg.sender_id);
        for (const recipient of recipients) {
          if (recipient.email) {
            await sendEmail(
              recipient.email,
              `Nouveau message de ${msg.sender_name || 'un collègue'}`,
              `Vous avez reçu un nouveau message de ${msg.sender_name || 'un collègue'} dans le canal staff. Connectez-vous pour le consulter.`
            );
          }
        }
    ...
    if (notifiedIds.length > 0) {
      ...
      await query(
        `UPDATE chat_messages SET notified = TRUE WHERE id IN (${placeholders})`,
        notifiedIds
      );
