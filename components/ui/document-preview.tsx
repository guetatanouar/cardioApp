...
export function DocumentPreview({ open, onOpenChange, filePath, fileName }: DocumentPreviewProps) {
  ...
  const isImage = /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(filePath) || /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(fileName);
  const isPdf = /\.pdf$/i.test(filePath) || /\.pdf$/i.test(fileName);

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent title={fileName} className="max-w-4xl w-[95vw] h-[90vh] flex flex-col">
