...
dashboardRouter.get("/summary", requireAuth, requireRole(["admin", "secretaire"]), requirePermission("can_view_patients"), async (_req, res) => {
    ...
    const [patientsCount, todayAppointments, unreadStaffMessages, criticalAlerts, recentActivity] = await Promise.all([
        ...
        query(`SELECT a.id, a.starts_at, a.duration_minutes, a.type, a.status, a.reason, a.notes,
         ...
         LIMIT 200`, [from, to]),
        query("SELECT count(*)::int as count FROM chat_messages WHERE channel = 'staff' AND is_read = false"),
        query(`SELECT p.id as patient_id, p.first_name, p.last_name, p.severity_status,
    ...
    const appointmentsRows = todayAppointments.rows;
