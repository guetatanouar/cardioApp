...
chatRouter.get("/", requireAuth, async (req, res) => {
    ...
    const result = await query(`SELECT id, channel, sender_role, sender_id, patient_id, content, created_at, is_read
     FROM chat_messages ${where}
     ORDER BY created_at ASC LIMIT 200`, params);
...
chatRouter.post("/", requireAuth, async (req, res) => {
    ...
    if (!canAccessChannel(req.user, parsed.data.channel))
        return res.status(403).json({ error: "FORBIDDEN" });
    const patientId = parsed.data.channel.startsWith("patient:")
        ? parsed.data.channel.slice("patient:".length)
        : null;
    const result = await query(`INSERT INTO chat_messages (channel, sender_role, sender_id, patient_id, content, is_read)
     VALUES ($1,$2,$3,$4,$5,false)
     RETURNING id`, [parsed.data.channel, req.user.role, req.user.id, patientId, parsed.data.content]);
